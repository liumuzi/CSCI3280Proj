...
# playnext havn't implemented
	...
	def getMusic(self, event):
	    ...
	    if self.filename[1].isdigit():
	    	self.sid = 10+int(self.filename[1])
	    else:
	    	self.sid = int(self.filename[0])
	    img2 = ImageTk.PhotoImage(Image.open(self.getAlbumPath(self.sid)))
	    albumBox.configure(image = img2) 
	    albumBox.image = img2
